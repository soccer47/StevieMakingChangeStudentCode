...
public class MakingChange {
    ...
    public static long countWays(int target, int[] coins) {
        ...
        for (int i = 0; i < coins.length; i++) {
            for (long j = 1; j < target + 1; j++) {
                ...
                // Otherwise check number of ways to reach the target by including the current coin is already known
                    ...
                    include = numCombos[(int)j - coinVals[i]][i];
                }
                ...
                // Otherwise check number of ways to reach the target by excluding the current coin is already known
                    ...
                    exclude = numCombos[(int)j][i - 1];
                }

                // Update the given index in numCombos with the sum of the possibilities of including and excluding
                // the current coin
                numCombos[(int)j][i] = include + exclude;
            }
        }
        ...
        return numCombos[target][coins.length - 1];
